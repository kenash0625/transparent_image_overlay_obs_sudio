...
obs_properties_t *get_properties_for_overlay(void *data)
{
    ...
    const QString filter_img;// = util_file_filter(T_FILTER_IMAGE_FILES, "*.jpg *.png *.bmp");
    const QString filter_text;// = util_file_filter(T_FILTER_TEXT_FILES, "*.json");
...
extern "C" __declspec(dllexport) void load_input()
{
    if (pInstance)
    {
